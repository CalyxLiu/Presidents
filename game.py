...
from newRandomPlayer import *
from simple_pomdp import *
from decks_and_cards import *
from simple_presRound import *
import math
import random
import pandas as pd
import ast
...
class Game(object):
    ...
    def __init__(self, numberOfPlayers, name, numRounds, passingRules, numDecks, restOfPlayers, toprint=False, newPlayer=Player, amountOfNewPlayers=1, anti=False, test=None, generateData=False):
        '''
        ...
        self.data = []
        self.numWins = [0, 0, 0, 0]
        self.numSecond = [0, 0, 0, 0]
        self.numThird = [0, 0, 0, 0]
        self.numFourth = [0, 0, 0, 0]
        ...
        for i in range(self.amountOfNewPlayers):
            '''
            commented out for random testing
            g = open("parameters-v4-8001-9000.txt", "r")
            N = ast.literal_eval(g.readline())
            Q = ast.literal_eval(g.readline())
            g.close()
            self.players.append(self.lastPlayer(i + self.numberOfPlayers - self.amountOfNewPlayers, defaultdict(lambda: 0, N), defaultdict(lambda: 0, Q)))
            self.lastPlayer = self.players[self.numberOfPlayers - 1]
            self.pomdpPlayer = self.players[self.numberOfPlayers - 1]
            '''
    ...
    def startGame(self):
        ...
        for i in range(self.numRounds):
            ...
            self.startingPlayer = random.choice(self.players)
            # starts the playing of rounds (this plays one round, the for loop above plays more rounds)
            newRound = presRound(self.players, self.startingPlayer, self.pomdpPlayer, self.print)
            ...
            if self.print:
                ...
                print(f"{i}: {printablePlayersOutOrder}")
                print("---------------------------")
                f.write(f"{i}: {printablePlayersOutOrder}\n")
            self.numWins[self.playersOutOrder[0].name] += 1
            self.numSecond[self.playersOutOrder[1].name] += 1
            self.numThird[self.playersOutOrder[2].name] += 1
            self.numFourth[self.playersOutOrder[3].name] += 1
        ...
        print(f"Second: {self.numSecond}")
        print(f"Third: {self.numThird}")
        print(f"Fourth: {self.numFourth}")

        if self.pomdpPlayer != None:
            g = open("parameters.txt", "w")
            N, Q = self.pomdpPlayer.getNandQ()
            g.write(str(dict(N)) + "\n")
            g.write(str(dict(Q)))
            g.close()
